...
static int WriteNakedMPQHeader(TMPQArchive * ha)
{
    ...
    Header.dwPassword     = pHeader->dwPassword;
...
bool WINAPI SFileCreateArchive(const TCHAR * szMpqName, DWORD dwCreateFlags, DWORD dwMaxFileCount, HANDLE * phMpq, LPBYTE pPassword)
{
    ...
    CreateInfo.dwStreamFlags  = (dwCreateFlags & STREAM_PROVIDER_MASK) | BASE_PROVIDER_FILE;
    ...
    return SFileCreateArchive2(szMpqName, &CreateInfo, phMpq, pPassword);
}

bool WINAPI SFileCreateArchive2(const TCHAR * szMpqName, PSFILE_CREATE_MPQ pCreateInfo, HANDLE * phMpq, LPBYTE pPassword)
{
    ...
    DWORD dwStreamProvider = pCreateInfo->dwStreamFlags & STREAM_PROVIDER_MASK;
    if(SFileOpenArchive(szMpqName, 0, dwStreamProvider | BASE_PROVIDER_FILE | MPQ_OPEN_NO_ATTRIBUTES | MPQ_OPEN_NO_LISTFILE, &hMpq))
    {
    ...
    if(pStream == NULL)
    {
        pStream = FileStream_CreateFile(szMpqName, pCreateInfo->dwStreamFlags, pPassword);
        if(pStream == NULL)
            return false;
    }
...
//  dwBlockTableSize = dwHashTableSize * 2;
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        pHeader->dwPassword       = pCreateInfo->dwPassword;
        ...
        ha->dwFlags |= (pCreateInfo->dwStreamFlags & STREAM_PROVIDER_MASK) & STREAM_PROVIDER_MPQE;
    }
