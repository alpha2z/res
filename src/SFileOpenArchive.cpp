    ...
    LPBYTE pPassword)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        pStream = FileStream_OpenFile(szMpqName, dwStreamFlags, pPassword);
        if(pStream == NULL)
            nError = GetLastError();
    }
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        while(SearchOffset < FileSize)
        {
            ...
            if (dwFlags & STREAM_PROVIDER_MPQE) {
                DecryptFileChunk(ha->HeaderData, ((TEncryptedStream *)ha->pStream)->Key, 0, MPQ_HEADER_SIZE_V1);
            }
