...
int SaveMPQTables(TMPQArchive * ha)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        if (ha->dwFlags & STREAM_PROVIDER_MPQE) {
            TEncryptedStream *pstEncryptStream = static_cast<TEncryptedStream *>(ha->pStream);
            assert(pHeader->dwHeaderSize == 64);
            EncryptFileChunk((LPDWORD)&SaveMpqHeader, pstEncryptStream->Key, 0, MPQ_HEADER_SIZE_V1);
        }
