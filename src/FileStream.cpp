...
static bool BlockStream_Read(
    ...
    DWORD dwBytesToRead)                    // Number of bytes to read from the file
{
    ULONGLONG uBeforeCurrent;
    ULONGLONG uCurrentPos;
    ULONGLONG uStartPos;
    ULONGLONG uEndOffset;
    LPBYTE pOriginBlockBuffer;
    LPBYTE pBlockBuffer;
    DWORD BlockBufferOffset;                // Offset of the desired data in the block buffer
    DWORD dwBytesNeeded;                      // Number of bytes that really need to be read
    ...
    uStartPos = (pByteOffset != NULL) ? *pByteOffset : pStream->StreamPos;
    uEndOffset = uStartPos + dwBytesToRead;
    if(uEndOffset > pStream->StreamSize)
    {
        SetLastError(ERROR_HANDLE_EOF);
        return false;
    }
    ...
    uBeforeCurrent = uCurrentPos = uStartPos & ~((ULONGLONG)dwBlockSize - 1);
    dwBlockCount  = (DWORD)(((uEndOffset - uCurrentPos) + (dwBlockSize - 1)) / dwBlockSize);
    dwBytesNeeded = (DWORD)(uEndOffset - uCurrentPos);

    // Remember where we have our data
    assert((dwBlockSize & (dwBlockSize - 1)) == 0);
    BlockBufferOffset = (DWORD)(uStartPos & (dwBlockSize - 1));

    // Allocate buffer for reading blocks
    pOriginBlockBuffer = pBlockBuffer = STORM_ALLOC(BYTE, (dwBlockCount * dwBlockSize));
    if(pOriginBlockBuffer == NULL)
    {
    ...
    if(pStream->IsComplete == 0)
    {
        ...
        bPrevBlockAvailable = pStream->BlockCheck(pStream, uCurrentPos);

        // Loop as long as we have something to read
        while(uCurrentPos < uEndOffset)
        {
            // Determine availability of the next block
            bBlockAvailable = pStream->BlockCheck(pStream, uCurrentPos);
            ...
            if(bBlockAvailable != bPrevBlockAvailable)
            {
                ...
                {
                    pStream->pfnCallback(pStream->UserData, uBeforeCurrent, (DWORD)(uCurrentPos - uBeforeCurrent));
                    bCallbackCalled = true;
                }

                // Load the continuous blocks with the same availability
                assert(uCurrentPos > uBeforeCurrent);
                bResult = pStream->BlockRead(pStream, uBeforeCurrent, uCurrentPos, pBlockBuffer, dwBytesNeeded, bPrevBlockAvailable);
                if(!bResult)
                    break;

                // Move the block offset
                pBlockBuffer += (DWORD)(uCurrentPos - uBeforeCurrent);
                dwBytesNeeded -= (DWORD)(uCurrentPos - uBeforeCurrent);
                bPrevBlockAvailable = bBlockAvailable;
                uBeforeCurrent = uCurrentPos;
            }

            // Move to the block offset in the stream
            uCurrentPos += dwBlockSize;
        }

        // If there is a block(s) remaining to be read, do it
        if(uCurrentPos > uBeforeCurrent)
        {
            // Call the file stream callback, if the block is not available
            if(pStream->pMaster && pStream->pfnCallback && bPrevBlockAvailable == false)
            {
            ...
            if(uCurrentPos > pStream->StreamSize)
                uCurrentPos = pStream->StreamSize;
            bResult = pStream->BlockRead(pStream, uBeforeCurrent, uCurrentPos, pBlockBuffer, dwBytesNeeded, bPrevBlockAvailable);
        }
    }
    else
    {
        // Read the complete blocks from the file
        if(uEndOffset > pStream->StreamSize)
            uEndOffset = pStream->StreamSize;
        bResult = pStream->BlockRead(pStream, uCurrentPos, uEndOffset, pBlockBuffer, dwBytesNeeded, true);
    }

    // Now copy the data to the user buffer
    if(bResult)
    {
        memcpy(pvBuffer, pOriginBlockBuffer + BlockBufferOffset, dwBytesToRead);
        pStream->StreamPos = uStartPos + dwBytesToRead;
    }
    else
    {
 ...
 * \a NewFileSize New size of the file
 */
static bool MpqeStream_ReSize(TFileStream * pStream, ULONGLONG NewFileSize)
{
#ifdef PLATFORM_WINDOWS
    {
        LONG FileSizeHi = (LONG)(NewFileSize >> 32);
...
static bool MpqeStream_GetSize(TFileStream * pStream, ULONGLONG * pFileSize)
{
    *pFileSize = pStream->Base.File.FileSize;
    return true;
}

static bool MpqeStream_GetPos(TFileStream * pStream, ULONGLONG * pByteOffset)
{
    *pByteOffset = pStream->Base.File.FilePos;
    return true;
}

static void MpqeStream_Close(TBlockStream * pStream)
{
    // Free the data map, if any
    if(pStream->FileBitmap != NULL)
        STORM_FREE(pStream->FileBitmap);
    ...
    DWORD dwStreamFlags)
{
    ...
    if(pStream != NULL)
    {
        ...
        //  add my logic
        if ((dwStreamFlags & STREAM_PROVIDER_MASK) == STREAM_PROVIDER_MPQE) {
            pStream->StreamSize = StreamSize + FileNameSize + sizeof(TCHAR);
        }
        
        if (dwStreamFlags & STREAM_PROVIDER_MASK & STREAM_PROVIDER_MPQE) {
            TEncryptedStream *pstEncrypt = static_cast<TEncryptedStream *>(pStream);
            pstEncrypt->StreamSize = StreamSize + FileNameSize + sizeof(TCHAR);
            pstEncrypt->BlockSize = MPQE_CHUNK_SIZE / 4;
        }
...
static bool MpqeStream_BlockCheck(
    TBlockStream * pStream,                // Pointer to an open stream
    ...
    DWORD dwLength)
{
    ...
    DWORD KeyShuffled[0x10] = {0};
    DWORD KeyMirror[0x10] = {0};
...
static bool MpqeStream_DetectFileKey(TEncryptedStream * pStream, LPBYTE pPassword)
{
    ULONGLONG uReadStart = 0;//ByteOffset = 0;
    BYTE arrEncryptedHeader[MPQE_CHUNK_SIZE] = {0};
    BYTE arrFileHeader[MPQE_CHUNK_SIZE] = {0};

    // Read the first file chunk
    if(pStream->BaseRead(pStream, &uReadStart, arrEncryptedHeader, sizeof(arrEncryptedHeader)))
    {
...
//            memcpy(pStream->Key, mpq_key, 33);
        
            // Try to decrypt with the given key
            // memcpy(FileHeader, EncryptedHeader, MPQE_CHUNK_SIZE);
            // DecryptFileChunk((LPDWORD)FileHeader, mpq_key, uReadStart, MPQE_CHUNK_SIZE);
        assert(pPassword != NULL);
        memcpy(pStream->Key, pPassword, PASSWORD_LENGTH);
        memcpy(arrFileHeader, arrEncryptedHeader, MPQ_HEADER_SIZE_V1);
        DecryptFileChunk((LPDWORD)arrFileHeader, pStream->Key, 0, MPQ_HEADER_SIZE_V1);
        ...
        if(arrFileHeader[0] == 'M' && arrFileHeader[1] == 'P' && arrFileHeader[2] == 'Q')
        {
            // Update the stream size
            pStream->StreamSize = pStream->Base.File.FileSize;
...
void EncryptFileChunk(
    DWORD * MpqData,
    LPBYTE pbKey,
    ULONGLONG ByteOffset,
    DWORD dwLength)
{
    DecryptFileChunk(MpqData, pbKey, ByteOffset, dwLength);
    
}

static bool MpqeStream_BlockWrite(
    ...
    LPBYTE BlockBuffer)
{
    ...
    DWORD dwBytesToWrite = (DWORD)(EndOffset - StartOffset);
//    EncryptFileChunk((DWORD *)BlockBuffer, pStream->Key, StartOffset, dwBytesToWrite);
    if (pStream->BaseWrite(pStream, &StartOffset, BlockBuffer, dwBytesToWrite)) {
        return true;
    }
    ...
    DWORD dwBytesToWrite)
{
    ...
    ULONGLONG uByteStart = (pByteOffset != NULL) ? *pByteOffset : pStream->Base.File.FilePos;
    const ULONGLONG uEndByte = uByteStart + dwBytesToWrite;
//    ULONGLONG uCurrPos = uByteStart;
//    ULONGLONG uTempEnd = 0;
//
//    DWORD dwBlockSize = pStream->BlockSize;
//
//    bool bCallbackCalled = false;
...
#if defined(PLATFORM_MAC) || defined(PLATFORM_LINUX)
{
    while(uCurrPos < uEndByte) {
        if (pStream->pMaster && pStream->pfnCallback) {
            pStream->pfnCallback(pStream->UserData, uCurrPos, dwBlockSize);
            bCallbackCalled = true;
        }
        uTempEnd = uCurrPos + dwBlockSize;
        bResult = pStream->BlockWrite(pStream, uCurrPos, uTempEnd, (LPBYTE)pvBuffer);
        if (!bResult)
            break;

        pvBuffer += dwBlockSize;
        uCurrPos += dwBlockSize;
    }

    if (bCallbackCalled && pStream->pfnCallback)
        pStream->pfnCallback(pStream->UserData, uByteStart, dwBytesToWrite);
}
#endif
*/
...
#if defined(PLATFORM_MAC) || defined(PLATFORM_LINUX)
{
    ...
    return pStream->BlockWrite(pStream, uByteStart, uEndByte, pvBuffer);
...
static TFileStream * MpqeStream_Open(const TCHAR * szFileName, DWORD dwStreamFlags, LPBYTE pPassword)
{
    TEncryptedStream * pStream;
    ...
    if(MpqeStream_DetectFileKey(pStream, pPassword))
    {
    ...
    LPBYTE pPassword)
{
    ...
    // We only support creation of flat, local file, add support encrypt
    if((dwStreamFlags & (STREAM_PROVIDERS_MASK)) != (STREAM_PROVIDER_FLAT | BASE_PROVIDER_FILE)
       && (dwStreamFlags & STREAM_PROVIDER_MASK) != (STREAM_PROVIDER_MPQE | BASE_PROVIDER_FILE))
    {
        SetLastError(ERROR_NOT_SUPPORTED);
        return NULL;
    }
    ...
    size_t sz = sizeof(TBlockStream);
    if((dwStreamFlags & STREAM_PROVIDERS_MASK) & STREAM_PROVIDER_MPQE) {
        sz = sizeof(TEncryptedStream);
    }
    
    pStream = AllocateFileStream(szFileName, sz, dwStreamFlags);
    if(pStream != NULL)
    {
        // Attempt to create the disk file
        bool bCreate = BaseFile_Create(pStream);
        if(bCreate && ((dwStreamFlags & STREAM_PROVIDERS_MASK) & STREAM_PROVIDER_MPQE))
        {
            assert(pPassword != NULL);
            TEncryptedStream * pEncryStream = static_cast<TEncryptedStream *>(pStream);
            memcpy(pEncryStream->Key, pPassword, PASSWORD_LENGTH);
            
            pEncryStream->StreamWrite   = (STREAM_WRITE)MpqeStream_Write;
            pEncryStream->StreamResize  = (STREAM_RESIZE)MpqeStream_ReSize;
            pEncryStream->StreamGetSize = (STREAM_GETSIZE)MpqeStream_GetSize;
            pEncryStream->StreamGetPos  = (STREAM_GETPOS)MpqeStream_GetPos;
            pEncryStream->StreamClose   = (STREAM_CLOSE)MpqeStream_Close;

            pEncryStream->BlockWrite    = (BLOCK_WRITE)MpqeStream_BlockWrite;
            pEncryStream->BlockCheck    = (BLOCK_CHECK)MpqeStream_BlockCheck;
    ...
    LPBYTE pPassword)
{
    ...
    switch(dwStreamFlags & STREAM_PROVIDER_MASK)
    {
        ...
        case STREAM_PROVIDER_MPQE:
            return MpqeStream_Open(szFileName, dwStreamFlags, pPassword);
