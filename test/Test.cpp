...
#ifdef PLATFORM_MAC
#define WORK_PATH_ROOT "/Users/riddick/StormLib/test"
#endif
...
int main(int argc, char * argv[])
{
    ...
    printf("===== BEGIN =====\n");
    const char* filename = "StormLibTest_FileTableFull.mpq";
    if(nError == ERROR_SUCCESS)
        nError = TestCreateArchive_FillArchive(filename);
   
    printf("===== TestOpenArchive(filename) =====\n");
    if(nError == ERROR_SUCCESS)
        nError = TestOpenArchive(filename);

    printf("===== FindFiles(ForEachFile_OpenArchive, NULL) =====\n");
    if(nError == ERROR_SUCCESS)
        nError = FindFiles(ForEachFile_OpenArchive, NULL);

    printf("===== TestFileStreamOperations(filename, STREAM_FLAG_READ_ONLY) =====\n");
    if(nError == ERROR_SUCCESS)
        nError = TestFileStreamOperations(filename, STREAM_FLAG_READ_ONLY);

//    printf("===== TestCreateArchive_FileFlagTest(filename) =====\n");
//    if(nError == ERROR_SUCCESS)
//        nError = TestCreateArchive_FileFlagTest(filename);
